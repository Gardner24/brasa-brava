...
import { PurchaseMovementRequest } from '@brasa/shared-types';
...
import { useProducts, useWarehouses, useRegisterPurchase } from '@/lib/queries.ts';
...
export function PurchaseForm({ onSuccess, defaultProductId, defaultWarehouseId }: Props) {
  ...
  const mutation = useRegisterPurchase();
  const { serverError, captureServerError, clearServerError } = useServerError();

  const form = useZodForm(PurchaseMovementRequest, {
    defaultValues: {
  ...
  const onSubmit = async (values: PurchaseMovementRequest): Promise<void> => {
    clearServerError();
    try {
      const payload: PurchaseMovementRequest = {
        ...values,
        ...(values.notes ? { notes: values.notes } : {}),
      };
      // notes vacío no debe ir
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" noValidate>
      <FormField
        ...
        required
      >
        <Select id="purchase-product" {...form.register('productId')}>
      ...
      <FormField
        ...
        required
      >
        <Select id="purchase-warehouse" {...form.register('warehouseId')}>
      ...
      <FormField label={t('fields.notes')} htmlFor="purchase-notes" error={errors.notes?.message}>
        <Input id="purchase-notes" type="text" maxLength={500} {...form.register('notes')} />
      </FormField>
